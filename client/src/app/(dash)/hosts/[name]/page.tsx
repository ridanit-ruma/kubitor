 ...
 *
 * One page for a machine whether or not Kubernetes has heard of it. A host
 * outside the cluster has no kubelet figures and everything else is the same,
 * so the Kubernetes parts are absent rather than the page being a second copy.
 */
export default function HostDetailPage() {
  ...
  // A machine the kubelet reports is a cluster member; one only the agent knows
  // about is not, and every Kubernetes figure on this page is absent for it.
  // While nothing is reporting at all, assume a node — this page is reached
  // from Nodes far more often than from Hosts.
  const inCluster = current?.clusterNode ?? true;
    ...
    <div className="screen gap-3">
      <div className="flex flex-wrap items-center gap-x-3 gap-y-1">
        <Button asChild variant="ghost" size="sm">
          <Link href={inCluster ? '/nodes' : '/hosts'}>
            <ArrowLeft className="size-4" />
            {inCluster ? 'Nodes' : 'Hosts'}
          </Link>
        </Button>
        <h1 className="text-base font-semibold tracking-tight">{name}</h1>
        {!inCluster && <Badge variant="outline">Outside the cluster</Badge>}
      ...
      <div className="pane flex flex-col gap-3 pr-1">
        ...
        {!host && (
          <p className="pb-1 text-xs text-muted-foreground">
            ...
            is not reporting for this machine.
          </p>
        )}
      </div>
    </div>
